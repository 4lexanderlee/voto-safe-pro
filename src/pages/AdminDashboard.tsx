...
import { Shield, LogOut, BarChart3, Users, Vote, TrendingUp, Search, FileText, Plus, Edit, Trash2, BookText } from 'lucide-react'; // Importar BookText para el icono de Cédula
import { PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import ChatBot from '@/components/ChatBot';
import ElectionFormModal from '@/components/ElectionFormModal';
import BallotCard from '@/components/BallotCard'; // <--- IMPORTAR EL NUEVO BallotCard
...
const AdminDashboard = () => {
  ...
  // Nuevo estado para la cédula que se está viendo en detalle (si lo necesitas)
  // const [selectedBallot, setSelectedBallot] = useState<Election | null>(null);
  ...
  useEffect(() => {
    // Aquí actualizamos el estado en localStorage
  ...
  const handleSaveElection = (election: Election) => {
    if (electionToEdit) {
      ...
      setElections(elections.map(elec => elec.id === election.id ? election : elec));
    } else {
      // Añadir nueva elección
      setElections([...elections, election]);
    }
    setElectionToEdit(null); // Limpiar
  };

  const handleOpenCreateModal = () => {
    setElectionToEdit(null); // Asegurarse de que no esté en modo edición
  ...
  const handleDeleteElection = (id: string) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar esta elección? Esta acción no se puede deshacer.")) {
      setElections(elections.filter(elec => elec.id !== id));
    }
  };

  // Función para ver los detalles de una cédula (placeholder por ahora)
  const handleViewBallotDetails = (election: Election) => {
    alert(`Ver detalles de la cédula para: ${election.nombre}\nID: ${election.id}`);
    // Aquí puedes abrir otro modal, navegar a otra ruta, etc.
    // setSelectedBallot(election); // Si usas un estado para mostrar detalles
  };
  ...
  // Party vote distribution (Este bloque necesita ser revisado si las votaciones están vinculadas a IDs de elección)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-secondary/10">
      {/* Header */}
      <header className="h-20 border-b border-border/50 bg-background/95 backdrop-blur-md sticky top-0 z-40 shadow-sm">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card className="border-none shadow-lg bg-gradient-to-br from-success/10 via-background to-background hover:shadow-xl transition-shadow">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-foreground">{elections.filter(e => e.estado === 'activa').length}</div>
              <div className="flex items-center gap-2 mt-1">
                <FileText className="h-3 w-3 text-muted-foreground" />
                <p className="text-xs text-muted-foreground">
                  de {elections.length} totales
                </p>
              </div>
            </CardContent>
          </Card>
      ...
      <div className="container mx-auto px-4 pb-8">
        <Tabs defaultValue="elecciones" className="space-y-6">
          <TabsList className="sticky top-20 z-30 grid w-full grid-cols-4 h-12 bg-background p-1 shadow-md"> {/* <--- Modificado a grid-cols-4 */}
            <TabsTrigger value="estadisticas" className="data-[state=active]:bg-muted data-[state=active]:shadow-md">
              <BarChart3 className="h-4 w-4 mr-2" />
              Estadísticas
            </TabsTrigger>
            <TabsTrigger value="votantes" className="data-[state=active]:bg-muted data-[state=active]:shadow-md">
              <Users className="h-4 w-4 mr-2" />
              Votantes
            </TabsTrigger>
            <TabsTrigger value="elecciones" className="data-[state=active]:bg-muted data-[state=active]:shadow-md">
              <Vote className="h-4 w-4 mr-2" />
              Elecciones
            </TabsTrigger>
            {/* <--- NUEVA PESTAÑA CÉDULA DE VOTACIÓN */}
            <TabsTrigger value="cedulas" className="data-[state=active]:bg-muted data-[state=active]:shadow-md">
          ...
          <TabsContent value="elecciones" className="space-y-6">
            <Card className="border-none shadow-lg">
              <CardHeader className="bg-gradient-to-r from-accent/5 to-transparent">
                ...
                <CardDescription>Crea y administra los parámetros de tus procesos electorales.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {elections.length === 0 ? (
                  <p className="text-center text-muted-foreground py-4">
                    Aún no hay elecciones creadas. ¡Empieza creando una!
                  </p>
                ) : (
                  elections.map(election => (
                    <div 
                      key={election.id} 
                      className="border-2 border-primary/20 rounded-lg p-5 bg-gradient-to-br from-primary/5 to-transparent hover:border-primary/40 transition-colors"
                    >
                      <div className="flex items-start justify-between mb-4">
                        <h3 className="font-bold text-lg">{election.nombre}</h3>
                        <Badge className={
                          election.estado === 'activa' ? 'bg-success/10 text-success hover:bg-success/20' :
                          election.estado === 'pendiente' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300' : // Nuevo estilo para pendiente
                          'bg-muted text-muted-foreground'
                        }>
                          {election.estado === 'activa' ? 'Activa' : (election.estado === 'pendiente' ? 'Pendiente' : 'Finalizada')}
                        </Badge>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm mb-4">
                        ...
                        <div className="flex items-center gap-2 col-span-2 md:col-span-1">
                          ...
                          <div>
                            <span className="text-muted-foreground block text-xs">Periodo</span>
                            <span className="font-medium">
                              {format(new Date(election.fechaInicio), "dd/MM/yy")} - {format(new Date(election.fechaFin), "dd/MM/yy")}
                            </span>
                          </div>
                        </div>
                      </div>
                      <div className="flex gap-2 justify-end border-t pt-4">
                        <Button variant="outline" size="sm" onClick={() => handleOpenEditModal(election)}>
          ...
          {/* <--- NUEVA PESTAÑA PARA CÉDULAS DE VOTACIÓN --- */}
            ...
            <Card className="border-none shadow-lg">
              <CardHeader className="bg-gradient-to-r from-primary/5 to-transparent">
                <CardTitle className="flex items-center gap-2">
                  ...
                  Cédulas de Votación Activas
                </CardTitle>
                <CardDescription>Aquí puedes ver el estado de las cédulas de votación generadas por tus elecciones.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {elections.length === 0 ? (
                  <p className="text-center text-muted-foreground py-4">
                    No hay cédulas de votación disponibles. Crea una elección primero.
                  </p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {elections.map(election => (
                      <BallotCard 
                        key={election.id} 
                        election={election} 
                        onView={handleViewBallotDetails} 
                        // Puedes pasar onEdit y onDelete si quieres que también se pueda hacer desde aquí
          ...
          {/* <--- FIN DE NUEVA PESTAÑA CÉDULAS DE VOTACIÓN --- */}
      ...
      {/* Renderizar el modal de creación/edición de elección */}
      <ElectionFormModal
